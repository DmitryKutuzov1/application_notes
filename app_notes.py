...
def user_action(notebook):
    while True:
        print('\nВыберите действие:\n1 - Добавить заметку | 2 - Просмотр |'
              ' 3 - Изменить | 4 - Удалить | 5 - Поиск по дате | 0 - Завершить работу\n')
...
def add_new_note(file_path):
    with open(file_path, 'a', newline='', encoding='utf-8') as file:
        ...
        note_content = input('Введите тело заметки: ')
        ...
        writer.writerow(
            [note_id, note_title, note_content, first_date, lust_date])
    ...
    with open(file_path, 'r', newline='', encoding='utf-8') as file, \
        ...
        for row in reader:
            if row[0] == note_id:
                ...
                note_content = input("Введите новое тело заметки: ")
                now = datetime.datetime.now()
                row[1] = note_title
                row[2] = note_content
...
def search_notes_by_date(file_path):
    date_input = input("Введите дату (формат YYYY-MM-DD): ")
    try:
    ...
    print("Заметки сделанные в этот день:")
